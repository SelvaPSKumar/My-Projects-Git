...
class General_health_results extends MY_Controller {
	...
	public function general_health_result_details( $header_id) {
		$access_given = array(PATIENT);
		$this->role_and_access($access_given);
		$this->load->helper('common_helper');
		$sql1 = "SELECT * from ".ASSESSMENT_HEADER." WHERE id = ". $header_id. " AND assessment_type_id=7 AND patient_id = ".$this->session->userdata('id'). " AND is_completed = 1 AND is_obsolete = 0";
		...
		$data['content'] = 'test_results/general_health_result_details/general_health';
